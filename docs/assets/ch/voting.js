...
const MAXG = 5, MING = 3;
const name = i => PETS[i];
const chip = i => `<span class="vo-chip">${PETS[i]}</span>`;
const SWAP = '<svg class="vo-ico" viewBox="0 0 16 16" aria-hidden="true"><path d="M2 5h11M10 2l3 3-3 3M14 11H3M6 8l-3 3 3 3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const HOW = `Press a swap button ${SWAP} to swap two pets on a ballot, or type a new group size.`;
...
/* groups host: opts = {sizes, arrows, remove, names, mark}. One compact card per group. The ranking reads first choice
   to last, left to right; with opts.arrows a swap button sits between every two neighbours (data-p = the left one). */
function groupsHTML(groups, n, opts){
  return groups.map((g, gi) => {
    ...
    const rank = g.rank.map((c, pos) => {
      if (pos === n - 1) return chip(c);
      const nx = g.rank[pos + 1];
      return chip(c) + (opts.arrows
        ? `<button type="button" class="vo-sw" data-g="${gi}" data-p="${pos}" data-d="1" title="Swap ${PETS[c]} and ${PETS[nx]}" aria-label="${gname}: swap ${PETS[c]} and ${PETS[nx]}">${SWAP}</button>`
        : '<span class="vo-then" aria-hidden="true">›</span>');
    }).join('');
    return `<div class="vo-group${opts.mark === gi ? ' vo-mark' : ''}"><div class="vo-ghead"><span class="vo-gname">${gname}</span>${size}${rm}</div><div class="vo-rank" role="group" aria-label="${gname}: ballot, first choice to last">${rank}</div></div>`;
  }).join('');
}
...
function refocus(host, gi, pos){
  const b = host.querySelector(`button[data-g="${gi}"][data-p="${pos}"]`);
  if (b) b.focus();
}
...
function winnerHTML(res){
  if (res.winner !== null && res.winner !== undefined) return `<span class="vo-winner"><span class="vo-wlab">Winner</span>${name(res.winner)}</span>`;
  const tied = res.tied || [];
  const txt = tied.length === 2 ? `Tie: ${list(tied.map(name))}` : tied.length === 3 ? 'Three-way tie' : 'Tie';
  return `<span class="vo-winner vo-tie"><span class="vo-wlab">Result</span>${txt}</span>`;
}
function card(title, sub, head, body, wide){
  return `<div class="vo-card${wide ? ' vo-wide' : ''}"><div class="vo-chead"><span class="vo-rule">${title}</span>${head}</div><div class="vo-sub">${sub}</div>${body}</div>`;
}
function pluralityCard(n, r){
  return card('Plurality', 'Most first-place votes wins.', winnerHTML(r), scoreRows(r.first, n, r.winner));
}
function bordaCard(n, r){
  const w = Array.from({ length: n }, (_, i) => n - 1 - i).join(', ');
  return card('Borda count', `Points for 1st, 2nd, ...: ${w}.`, winnerHTML(r), scoreRows(r.pts, n, r.winner));
}
function irvCard(n, r){
  ...
  return card('Instant runoff', `Last place drops out until someone has more than half (${r.majority} of ${r.total}).`, winnerHTML(res), `<ol class="vo-rounds">${rows}</ol>`, true);
}
function condorcetCard(n, c){
  ...
  const head = c.winner !== null ? winnerHTML({ winner: c.winner, tied: [] }) : `<span class="vo-winner vo-tie"><span class="vo-wlab">Result</span>${c.cycle ? 'A cycle' : 'No winner'}</span>`;
  return card('Condorcet', 'Head to head. Read across: the row pet\'s votes, then the column pet\'s.', head, `${t}<div class="vo-verdict">${v}</div>`, true);
}
...
const st = { n: 3, groups: fromPreset(PRESET.three), loaded: 'three', sinceReset: false, hint: 0, say: '' };
...
function bResults(){
  ...
  $('#vo-results').innerHTML = pluralityCard(st.n, r.plurality) + bordaCard(st.n, r.borda) + irvCard(st.n, r.irv) + condorcetCard(st.n, r.condorcet);
  bStatus(r);
}
function bRender(){
  const n = st.n, groups = st.groups;
  BOARD.classList.toggle('vo-n4', n === 4);
...
/* The strip is the narrator: what the four rules say about this profile, then what to try next. */
function bStatus(r){
  const P = r.plurality.winner, I = r.irv.winner, B = r.borda.winner, C = r.condorcet;
  const lead = st.say ? st.say + ' ' : ''; st.say = '';
  if (total(st.groups) === 0){ turn(BOARD, 'math', `${lead}Nobody is voting yet. Type a number of students into a group's box.`); return; }
  ...
  if (three){
    ...
    if (isThreePreset) h += ` Press a swap button ${SWAP} or type a new group size, and keep all three disagreeing.`;
    else { earn('vo-three'); won = true; h += ' That is a three-way split of your own.'; }
    parts.push(h);
  }
  if (C.cycle){
    const cyc = C.cycle.concat([C.cycle[0]]).map(name).join(' beats ');
    let h = `No Condorcet winner: ${cyc}. A cycle.`;
    if (st.sinceReset){ earn('vo-cycle'); won = true; h += ' You built it from scratch.'; }
    else if (st.loaded === 'cycle') h += ' Break it: type new group sizes to move students until one pet beats both others. (The star is for building a cycle yourself after Reset.)';
    else h += ' To earn the star, press Reset and build a cycle yourself.';
    parts.push(h);
  }
  if (parts.length){ turn(BOARD, won ? 'win' : 'math', lead + parts.join(' '), won ? 'Star' : ''); return; }
  if (st.loaded === 'cycle' && C.winner !== null){
    const tot = total(st.groups);
    let h = `Cycle broken: <b>${name(C.winner)}</b> beats everyone.`, best = false;
    if (tot !== 30) h += ` But the class is now ${tot} students. Keep it at 30 and move students between groups instead.`;
    else {
      const moved = movedFrom(fromPreset(PRESET.cycle), st.groups);
      h += ` You moved ${moved} student${moved === 1 ? '' : 's'}.`;
      best = moved <= 6;
      h += best ? ' That is the fewest possible.' : ' Can you do it with fewer? Press A cycle to load it again.';
    }
    turn(BOARD, best ? 'win' : 'math', lead + h, best ? 'Solved' : ''); return;
  }
  const say = w => (w === null ? 'a tie' : name(w));
  const agree = P !== null && P === I && I === B && C.winner === P ? ' Everyone agrees.' : '';
  turn(BOARD, 'math', `${lead}Plurality: ${say(P)}. Instant runoff: ${say(I)}. Borda: ${say(B)}. Condorcet: ${C.winner === null ? 'nobody' : name(C.winner)}.${agree} ${HOW}`);
}
const LOADED = { three: 'The three-way split is loaded.', cycle: 'The cycle is loaded.', reset: 'Reset: three groups of 10 with the same ballot.' };
function bLoad(key){
  st.n = 3; st.groups = fromPreset(PRESET[key]); st.loaded = key; st.sinceReset = key === 'reset'; st.say = LOADED[key];
  $('#vo-hint-out').hidden = true;
  bRender();
}
wireGroups($('#vo-groups'), {
  move(gi, pos, d){
    const g = st.groups[gi]; if (!g || !moveInRank(g.rank, pos, d)) return;
    bRender(); refocus($('#vo-groups'), gi, pos);
  },
  remove(gi){
...
$('#vo-load-cycle').addEventListener('click', () => { bLoad('cycle'); BOARD.scrollIntoView({ behavior: WM.reduced ? 'auto' : 'smooth', block: 'start' }); });
$('#vo-add-group').addEventListener('click', () => {
  ...
  st.say = `Group ${st.groups.length} added with 0 students. Type its size.`;
  bRender();
  const inp = $(`#vo-groups input[data-g="${st.groups.length - 1}"]`); if (inp){ inp.focus(); inp.select(); }
});
$('#vo-cand').addEventListener('click', () => {
  if (st.n === 3){ st.n = 4; for (const g of st.groups) g.rank.push(3); st.say = 'Gecko joins, last on every ballot for now.'; }
  else { st.n = 3; for (const g of st.groups) g.rank = g.rank.filter(c => c !== 3); st.say = 'Gecko is off the ballot.'; }
  bRender();
});
const HINTS = [
...
const HINT_FOR = ['a three-way split', 'a cycle', 'the Challenge'];
/* A hint is a recipe with several steps, so it is pinned under the ballots and survives the edits it asks for. */
$('#vo-hint').addEventListener('click', () => {
  let i;
  if (!WM.has('vo-three')) i = 0; else if (!WM.has('vo-cycle')) i = 1; else i = 2;
  if (st.hint === i && !$('#vo-hint-out').hidden) i = (i + 1) % HINTS.length;   // a second click shows the next hint
  st.hint = i;
  const out = $('#vo-hint-out'); out.innerHTML = `<b>Hint.</b> ${HINTS[i]}`; out.hidden = false;
  turn(BOARD, 'math', `A recipe for ${HINT_FOR[i]} is pinned under the ballots. It stays there while you edit. Press Hint again for the next one.`, 'Hint');
  WM.reveal(out);
});
bRender();
cue($$('#vo-groups .vo-group:first-child .vo-sw'));
...
function s1Render(mode){
  ...
  if (mode === 'honest') turn('#vo-s1-board', 'math', `<b>${name(r.winner)}</b> wins with ${r.first[r.winner]} first-place votes. The Fish group gets its last choice. Press <b>Fish group votes strategically</b> and watch the winner change.`, 'Honest votes');
  else turn('#vo-s1-board', 'math', `<b>${name(r.winner)}</b> wins with ${r.first[r.winner]}. The Fish group ranks Turtle above Hamster, so all 8 of them are happier, and not one of them voted for Fish. Press <b>Fish group votes honestly</b> to switch back.`, 'Strategic votes');
}
seg($('#vo-s1'), s1Render);
s1Render('honest');
...
function s2Render(){
  ...
  if (changed.length === 0){ turn(S2B, 'you', `Honest votes: <b>${name(hw)}</b> wins with ${r.pts[hw]} points. Press a swap button ${SWAP} on one group's ballot, so that group ends up with a pet it likes more than ${name(hw)}.`); return; }
  if (changed.length > 1){ turn(S2B, 'you', 'Change just one group at a time. Two groups have changed: swap one of them back, or press Back to the honest votes.', 'Not yet'); return; }
  const gi = changed[0], gname = S1NAMES[gi], honestRank = s2Honest[gi].rank;
  if (r.winner === null){ turn(S2B, 'you', `A tie: ${list(r.tied.map(name))}. Not a win for anyone yet. Keep swapping.`, 'Not yet'); return; }
  if (r.winner === hw){ turn(S2B, 'you', `<b>${name(hw)}</b> still wins with ${r.pts[hw]} points. The ${gname} is no better off. Try another swap, or another group.`, 'Not yet'); return; }
  if (honestRank.indexOf(r.winner) < honestRank.indexOf(hw)){
    turn(S2B, 'win', `Now <b>${name(r.winner)}</b> wins, and the ${gname} honestly likes ${name(r.winner)} more than ${name(hw)}. They got a better result by ranking dishonestly. That is a strategic vote, and this one has a name: burying.`, 'Solved');
    earn('vo-strategic');
  } else {
    turn(S2B, 'you', `Now <b>${name(r.winner)}</b> wins, but the ${gname} likes ${name(r.winner)} even less than ${name(hw)}. That backfired. Swap it back and try something else.`, 'Backfired');
  }
}
wireGroups($('#vo-s2-groups'), {
  move(gi, pos, d){
    const g = s2.groups[gi]; if (!g || !moveInRank(g.rank, pos, d)) return;
    s2Render(); refocus($('#vo-s2-groups'), gi, pos);
  },
  remove(){},
});
const S2HINTS = [
...
$('#vo-s2-hint').addEventListener('click', () => {
  turn(S2B, 'you', S2HINTS[s2.hint], 'Hint');
