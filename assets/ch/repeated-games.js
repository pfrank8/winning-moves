...
function guess(key){
  ...
  if (right && (actual.random || others.length === 0)){
    ...
    msg = `<span class="you">Not this time.</span> Robo was playing <b class="robo">${actual.name}</b>, not ${yours ? yours.name : key}.` +
      (fits ? ` Your guess would have played the same 20 moves, so your moves did not separate them. A single grab, followed by shares, usually does.` : ` Look at the rounds after your grabs: that is where the strategies differ.`);
  }
...
const HINTS = {
  ...
  mystery: 'Probe it. Share three times, grab once, then share three times, and watch the answer. Tit for Tat grabs back once. Grudger grabs forever. Tit for Two Tats lets one grab go. Pavlov grabs back until that stops paying. Always Share never grabs. Random does not follow your moves at all.',
};
...
function runTournament(fromButton){
  ...
  const rows = list.map((s, i) => {
    ...
    return { s, total: res.total[i], h2h };
  }).sort((a, b) => b.total - a.total);
  const max = rows[0].total || 1;
  let h = '<tr><th>#</th><th>Strategy</th><th>Total</th><th>Per round</th><th>vs Tit for Tat</th><th></th></tr>';
  rows.forEach((r, i) => {
...
seg($('#rg-b-first'), v => { t.first = v; describeBuild(); });
seg($('#rg-b-s'), v => { t.afterS = v; describeBuild(); });
seg($('#rg-b-g'), v => { t.afterG = v; describeBuild(); });
